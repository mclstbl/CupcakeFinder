...
// Use PDO to connect to the database.
        ...
        $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
// Prepare the query.
        $query = $db->prepare('SELECT * FROM places
            LEFT JOIN reviews
            ON places.place_id=reviews.place_id
            WHERE stars >= :stars
            ');
// Execute the query using passed in username and password values as parameters.
        global $stars;
...
function printSearchResults($row) {
...
// Prepare the insert query.
    ...
    ");
// Execute the query using passed in values as parameters.
    if ($clothing == null) $clothing = 0;
    if ($electronics == null) $electronics = 0;
    if ($food == null) $food = 0;
    $query->execute(array('place_id'=>$place_id, 'placename'=>$placename,
    ...
    $redirect_page =  'https://{$_SERVER["HTTP_HOST"]}/public\/individual.php?place_id=' . $place_id;
